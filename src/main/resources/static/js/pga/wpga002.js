    ...
    form(obj).addEventListener("click", function () {
        switch (obj) {
            ...
            case  "bgerarbol": form("sacao").innerText   = "Gerando";
                               form("stitulo").innerText = form("sacao").innerText + " Cobranca ";
...
function filaFetchInit(){
    CONSUL.filaFetch = (retorno,erro)=>{
        ...
        switch (CONSUL.obj) {
        case "registrarFatura": alert("Boleto gerado com sucesso!","O boleto foi gerado para o contrato. O beneficiario tem até 15 dias após a emissão do boleto para realizar o pagamento",4);
                                break;
...
function controlaTela(opc){
    ...
    if(opc == "modal"){
        //desabilitaCampo('mdatavenc',    ehConsultando());
        desabilitaCampo('mcodcontrato', ehConsultando());
        desabilitaCampo('bgerar',       ehConsultando());
...
function registrarFatura(){
    CONSUL.consultar("registrarFatura", `/faturas/registrarFatura`,['codcontrato:mcodcontrato'],"POST");
}
...
function ehConsultando(){
    return form("sacao").innerText == "Consultando";
}